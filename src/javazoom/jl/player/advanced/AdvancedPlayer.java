...
//				System.out.println(audio.getPosition());
	...
	// ADDITIONAL METHODS FOR GETTING CURRENT LOCATION IN FRAMES
	// These methods are used for the current frame location into the
	// song file the player is for server sends.
	/**
	 * get the current playback position in frames
	 * 
	 * @return current frame location of song, or 0 if no song is playing
	 */
	public int getFrameLocation()
	{
		int location = 0;
		if(audio != null)
			location = audio.getPosition();
		return location;
	}
}